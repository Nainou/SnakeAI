...
class DQNAgent:
    ...
    def save(self, filepath, include_metadata=True):
        ...
        # Pixel model: ConvQN with feat_dim=64, extra_dim=8, hidden=128, output=3
        # The head network is: (64+8) -> 128 -> 3
        input_size = 72  # feat_dim + extra_dim = 64 + 8
        hidden_size = self.q_network.head[0].out_features  # 128
        ...
        if include_metadata:
            ...
            # Format: pixel_{input_size}_{hidden_size}_{output_size}_{extra_info}.pth
            parts = ["pixel", str(input_size), str(hidden_size), str(output_size)]
            if extra_info:
...
def train_agent(episodes=1000, batch_size=32, update_target_every=20):
    ...
    for episode in range(episodes):
        ...
        if episode % 500 == 0 and episode > 0:
            ...
            agent.save(f'pixel/saved/snake_model_episode_{episode}.pth')
...
if __name__ == "__main__":
    ...
    agent.save('pixel/saved/snake_model_final.pth')
    # Get the actual saved filename
    from pathlib import Path
    saved_dir = Path('pixel/saved')
    final_models = list(saved_dir.glob('pixel_*_final.pth'))
    if final_models:
        print(f"\nModel saved as '{final_models[0].name}'")
    else:
        print("\nModel saved (check pixel/saved/ directory)")
